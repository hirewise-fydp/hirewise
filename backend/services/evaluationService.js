  ...
  try {
    ...
    if (application.status === "test_invited") {
      try {
        ...
        console.log(
          `Test invitation email sent to ${application.candidateEmail}`
        );
      } catch (emailError) {
        console.error(
          `Failed to send test invitation email to ${application.candidateEmail}:`,
          emailError
        );
      }
...
export const evaluateTestAnswers = async (applicationId) => {
  ...
  try {
    ...
    try {
      ...
      console.error(
        `Failed to send test completion email to ${application.candidateEmail}:`,
        emailError
      );
    }
    ...
    if (
      typeof evaluation.testScore !== "number" ||
      evaluation.testScore < 0 ||
      evaluation.testScore > 100
    ) {
    ...
    console.log(
      `Test evaluation completed for application ${applicationId}: Score ${evaluation.testScore}`
    );
    ...
    try {
      if (evaluation.testScore >= 75) {
        ...
        console.log(
          `Test rejection email sent to ${application.candidateEmail}`
        );
      }
    } catch (emailError) {
      console.error(
        `Failed to send email to ${application.candidateEmail}:`,
        emailError
      );
    }
  } catch (error) {
    console.error(
      "Test evaluation failed for application",
      applicationId,
      error
    );
    await CandidateApplication.findByIdAndUpdate(applicationId, {
    ...
    if (application) {
      try {
        ...
        console.error(
          `Failed to send evaluation failure email to ${application.candidateEmail}:`,
          emailError
        );
      }
    }
...
async function calculateScores(parsedResume, job) {
  ...
  const formattedEvaluation = {
    ...
    skillMatches: (evaluation.evaluationResults.skillMatches || [])
      .map((skill) => ({
        skill: skill.name || skill.skill || "",
        matchStrength: skill.confidence || skill.matchStrength || 0,
      }))
      .filter(
        (skill) => skill.skill && typeof skill.matchStrength === "number"
      ),
    experienceScore:
      evaluation.evaluationResults.experience ||
      evaluation.evaluationResults.experienceScore ||
      0,
    educationScore:
      evaluation.evaluationResults.education ||
      evaluation.evaluationResults.educationScore ||
      0,
    overallScore:
      evaluation.evaluationResults.overall ||
      evaluation.evaluationResults.overallScore ||
      0,
  };
